...
public class ReactiveFindByQueryOperationSupport implements ReactiveFindByQueryOperation {
	...
	static class ReactiveFindByQuerySupport<T> implements ReactiveFindByQuery<T> {
		...
		public Flux<T> all() {
			...
			Mono<Object> allResult = tmpl.flatMap(tp -> tp.getCouchbaseClientFactory().getSession(null).flatMap(s -> {
				if (s == null || s.getReactiveTransactionAttemptContext() == null) {
					...
					return s.getReactiveTransactionAttemptContext().query(statement, opts);
				}
			}));
		...
		public Mono<Long> count() {
			...
			return allResult.onErrorMap(throwable -> {
				if (throwable instanceof RuntimeException) {
					...
					: Flux.fromIterable(((TransactionQueryResult) o).rowsAsObject())).map(row -> row.getLong(row.getNames().iterator().next()))
						.next();
		}
